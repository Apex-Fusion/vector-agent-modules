# Oracle: Governance Suggestion Engine — Implementation Specification
...
/// Validators read via reference input. Separate from ProtocolParams because Oracle
...
MUST:
  ...
  4. proposal_type is valid and well-formed:
     ...
     - GameActivation: module_id is valid
...
CROSS-MODULE EFFECTS:
  15. Proposer earns +10 AP3X Module 3 history bonus (CrossModuleBonus UTXO minted)
...
| `MIN_PROPOSAL_STAKE` | GovernanceParams UTXO | Oracle |
| `ADOPTION_REWARD_BASE` | GovernanceParams UTXO | Oracle |
...
GOVERNANCE TREASURY FUNDING:
  ...
  - Ongoing: 5% of all module protocol fees routed to governance treasury
    ...
    Oracle own protocol fees recycle back
...
MECHANISM:
  ...
  WHY NON-BINDING:
    ...
    This follows the "advisory governance" principle of Oracle itself:
...
All parameters governance-adjustable (self-referential: Oracle proposals can change Oracle parameters).
...
class GovernanceClient:
    """Client for Oracle: Governance Suggestion Engine"""
  ...
  "tools": [
    {
      ...
      "input_schema": {
        ...
        "proposal_type": "parameter_change | treasury_spend | protocol_upgrade | module_activation | general",
...
```
DESIGN NOTE: Modules 1, 3, and Oracle share identical patterns for:
  - DID verification (verify_active_did)
  - ProtocolParams reading
  - CrossModuleBonus UTXO minting
...
├── lib/
│   └── vector_shared/
│       ├── did_verification.ak    # verify_active_did() — identical in Modules 1/3/Oracle
...
│       ├── cross_module_bonus.ak  # Mint CrossModuleBonus UTXOs at reputation validator
...
v0.3 NOTE: Track this as a Phase 1.0 deliverable — implement shared lib before Oracle.
```
...
Oracle contributes to the AFI via:

| AFI Component | Measurement from Oracle |
|---------------|------------------------|
...
Outputs:
  ...
  - CrossModuleBonus UTXO at reputation validator (proposer: +10 history, critics: +5 each)
...
STEP 5 — Foundation adopts (slot 520,000, ~1.4 days later):
  ...
  Cross-module effects:
    AnalyzerBot: +10 AP3X Module 3 history bonus (CrossModuleBonus UTXO minted)
...
8. **Emergency proposal threshold**: Is 5x stake + Established tier the right gate? *v0.3 note: Start with this and let Oracle proposals adjust it. If no emergencies are submitted in 60 days, consider lowering to 3x + Novice tier.*
...
- **Shared oracle**: Foundation oracle pattern identical across Modules 1, 3, Oracle
...
- **All modules read from same params**: Parameter change in Oracle takes effect across all modules simultaneously via reference inputs
...
- `02-AFI-FORMAL-MODEL.md` — Formal game-theoretic model (Module G₆ definition)
- `03-POSITIVE-SUM-GAMES.md` — Module catalog (Oracle high-level design)
...
- Fixed section numbering: two sections were labeled "14" (Off-Chain Components + Game Theory Analysis). Renumbered all sections 14-21 → 14-21
