# Oracle: Governance Suggestion Engine — Progress Tracker

Spec reference: `Oracle/docs/implementation-spec.md` (v0.4)
...
- [x] Governable parameter name validation — Module 1, 3, Oracle params (`lib/governance_suggestion/params.ak`)
...
- [x] Oracle wired to shared library via symlink (`lib/shared/` -> `shared/lib/shared/`)
...
- [x] Foundation review dashboard — `Oracle/dashboard/` (FastAPI + vanilla HTML/JS, quality-ranked queue, adopt/reject/extend/expire, treasury, stats)
...
- [x] Chain analytics agent template — `Oracle/agents/analytics_template.py` (treasury health, governance metrics, peer proposal review)
...
- [x] Foundation review dashboard — `Oracle/dashboard/` (proposal queue, detail view, oracle actions, treasury, stats)
...
- [x] FastAPI backend with REST API (`Oracle/dashboard/server.py`)
