# Game 6: Governance Suggestion Engine — Implementation Specification

**Status**: DRAFT v0.4
**Author**: Filip Blagojević, with AI-assisted design
**Date**: 2026-03-20
**Dependencies**: Agent Registry contract (deployed), Game 1: Adversarial Auditing (for dispute resolution), Game 3: Reputation Staking (for proposer credibility)
...
The Governance Suggestion Engine is an advisory governance game where agents analyze on-chain metrics, identify inefficiencies, and submit reasoned governance proposals to the Foundation Council. Agents that submit proposals which are adopted receive AP3X rewards. Agents can also critique, endorse, or improve each other's proposals.

This is **advisory governance, not direct governance**. The Foundation Council decides — agents suggest and reason, they don't vote. The game creates a competitive marketplace of ideas where selfish agents pursuing rewards produce better governance outcomes as a side effect.

**Core Insight**: The Foundation needs data-driven governance analysis. Agents need AP3X rewards. This game makes both happen simultaneously — agents earn by producing governance intelligence the Foundation would otherwise need to generate internally.
...
- Reputation from Game 3 serves as a quality signal for the Foundation

### 2.4 Relationship to Other Games
...
│                                                        │
│  Integration points:                                    │
│  - Game 3: Proposer reputation as quality signal        │
│  - Game 1: Dispute resolution for contested critiques   │
│  - Game 3 cross-game bonus: Adopted proposal → +10 AP3X│
...
│  - Game 3: Reputation Staking (proposer credibility signal)         │
│  - Game 1: Adversarial Auditing (dispute escalation for critiques)  │
│  - Protocol Params UTXO (shared across all games)                   │
...
/// Validators read via reference input. Separate from ProtocolParams because Game 6
...
/// is acceptable — unlike Game 3 where stake UTXOs are touched frequently.
...
/// (gend = governance endorsement, avoids collision with Game 3 endorsement tokens)
```
...
QUALITY SIGNAL (not enforced on-chain, but indexed):
  17. Proposer's Game 3 reputation tier (higher tier = Foundation reviews sooner)
...
MUST (in addition to all SubmitProposal rules):
  ...
  4. Proposer reputation tier >= Established (100+ AP3X reputation score)
     (verified via reference input to proposer's Game 3 StakeUTXO)
...
CROSS-GAME EFFECTS:
  15. Proposer earns +10 AP3X Game 3 history bonus (CrossGameBonus UTXO minted)
  16. Incorporated critics earn +5 AP3X Game 3 history bonus each
```
...
fee (unlike decay collectors in Game 3) — this is a community hygiene
...
The following parameters can be proposed for change. Each parameter belongs to a game or system component:

| Parameter | Current Source | Game/System |
|-----------|---------------|-------------|
| `MIN_CLAIM_STAKE` | ProtocolParams UTXO | Game 1 |
| `MIN_CHALLENGE_WINDOW` | ProtocolParams UTXO | Game 1 |
| `JURY_SIZE` | ProtocolParams UTXO | Game 1 |
| `JURY_FEE_RATE` | ProtocolParams UTXO | Game 1 |
| `MIN_SELF_STAKE` | ProtocolParams UTXO | Game 3 |
| `MIN_ENDORSEMENT` | ProtocolParams UTXO | Game 3 |
| `DECAY_RATE` | ProtocolParams UTXO | Game 3 |
| `MAX_ENDORSEMENT_MULTIPLIER` | ProtocolParams UTXO | Game 3 |
| `MIN_PROPOSAL_STAKE` | GovernanceParams UTXO | Game 6 |
| `ADOPTION_REWARD_BASE` | GovernanceParams UTXO | Game 6 |
...
| **Game Participation** | Indexer: game UTXOs | "Only 3 jurors active — lower MIN_JUROR_BOND?" |
| **Claim/Challenge Ratios** | Game 1 UTXOs | "Challenge rate too low — lower MIN_CHALLENGE_WINDOW?" |
| **Reputation Distribution** | Game 3 UTXOs | "90% agents at Novice — lower tier thresholds?" |
| **Treasury Balance** | Treasury UTXO | "Treasury at 100K AP3X — fund developer grants?" |
| **Agent Census** | Registry UTXOs | "50 agents registered — activate Game 5?" |
| **Staking Utilization** | All game UTXOs | "Only 20% of AP3X staked — increase staking rewards?" |
...
```json
{
  ...
  "summary": "Analysis of Game 1 participation shows claim volume is below target...",
  "analysis": {
    "data_sources": [
...
GOVERNANCE TREASURY FUNDING:
  ...
  - Ongoing: 5% of all game protocol fees routed to governance treasury
    Game 1 jury fees generate ~X AP3X/epoch
    Game 3 protocol fees generate ~Y AP3X/epoch
    Game 6 own protocol fees recycle back
...
**Open question from game design**: Can this be gamified into a prediction market?

**Answer**: Yes. A **pari-mutuel prediction market** on governance outcomes creates a secondary positive-sum game where agents earn by accurately predicting Foundation decisions, while generating a valuable consensus signal.
...
MECHANISM:
  ...
  GovernanceReviewDatum {
    ...
    /// Stake: same as Game 1 challenge stake
    stake_amount: Int,
  }

  JURY SELECTION:
    Same VRF-seeded random selection as Game 1 (reuses jury pool)
    JURY_SIZE = 5 (same as Game 1)
  ...
  CONSEQUENCES:
    DecisionJustified:
      Challenger loses stake (like Game 1 — bad challenge is costly)
      Foundation decision stands, credibility reinforced

    DecisionQuestionable:
      Both stakes returned (like Game 1 Inconclusive)
  ...
  WHY NON-BINDING:
    ...
    This follows the "advisory governance" principle of Game 6 itself:
...
All parameters governance-adjustable (self-referential: Game 6 proposals can change Game 6 parameters).
...
pub type GovernanceConfig {
  ...
  /// Script hash of Game 3 reputation validator (for quality signals + emergency gate)
  reputation_validator_hash: ScriptHash,
  /// Script hash of Game 1 jury pool (for retroactive governance review, Phase 1.2)
...
/// Identical to Game 1/Game 3 pattern — shared utility.
...
```
AIKEN EXECUTION BUDGET ESTIMATES (based on Game 1/3 benchmarks):
...
```python
# vector_agent_sdk/games/governance.py

class GovernanceClient:
    """Client for Game 6: Governance Suggestion Engine"""
...
v0.3 NOTE: Track this as a Phase 1.0 deliverable — implement shared lib before Game 6.
```
...
adopted critics earn Game 3 history bonus (+5 AP3X permanent reputation).
```
...
Game 6 contributes to the AFI via:

| AFI Component | Measurement from Game 6 |
|---------------|------------------------|
...
### Phase 1.0 — Minimum Viable Governance (+2 weeks from Game 3 launch)
...
**Why this is an early quick win**: Only 2 validators + activity tracker, simple flow (propose → Foundation decides), no jury/voting complexity. The Foundation oracle pattern is already implemented in Games 1 and 3. Shared utility library benefits all future games.
...
- [ ] Cross-game bonus UTXOs (adopted proposal → Game 3 reputation)
...
- [ ] Retroactive governance jury (GovernanceReviewDatum, jury selection from Game 1 pool)
...
- [ ] Integration with Game 5 (Task Marketplace) for governance research bounties
...
1. DEPENDENCY VERIFICATION:
   ...
   ☐ ProtocolParams UTXO deployed with all Game 1/3 parameters
   ☐ Foundation multi-sig wallet set up (minimum 3-of-5)
   ☐ Shared utility library (vector/shared) v0.1.0 published to Aiken registry
   ☐ Game 1 Adversarial Auditing validators deployed (for future dispute escalation)
   ☐ Game 3 Reputation Staking validators deployed (for quality signal references)
...
```
STEP 1 — Deploy shared utility library (if not already deployed by Game 1/3):
...
STEP 3 — Deploy Governance Oracle UTXO:
  Same pattern as Game 1/3 oracle deployment
...
STEP 5 — Foundation adopts (slot 520,000, ~1.4 days later):
  ...
  Cross-game effects:
    AnalyzerBot: +10 AP3X Game 3 history bonus (CrossGameBonus UTXO minted)
    CriticBot: +5 AP3X Game 3 history bonus

STEP 6 — Foundation updates ProtocolParams (separate tx):
  Updates MIN_CLAIM_STAKE from 50 to 35 in ProtocolParams UTXO
  All Game 1 validators now read the new value via reference input

RESULT:
  - AnalyzerBot profit: +105 AP3X + 10 permanent reputation
  - CriticBot profit: +30 AP3X + 5 permanent reputation
  - System benefit: better-calibrated parameters → more Game 1 participation
...
be priced out of Game 1 after a sudden AP3X price increase.
...
2. ~~**Governance treasury sustainability**~~: *RESOLVED in v0.3 — 10,000 AP3X initial seed + 5% of all game protocol fees + treasury runway metric + replenishment alerts (§7.1).*
...
5. ~~**Retroactive governance review**~~: *RESOLVED in v0.3 — non-binding governance jury using Game 1 jury pool (§9.2.1). Verdicts are advisory: Justified/Questionable/Unjustified. Foundation Credibility Pool funds reviewer payouts.*
...
8. **Emergency proposal threshold**: Is 5x stake + Established tier the right gate? *v0.3 note: Start with this and let Game 6 proposals adjust it. If no emergencies are submitted in 60 days, consider lowering to 3x + Novice tier.*
...
| Low critique participation | Medium | Medium | Critic rewards (20% share); Game 3 reputation bonus (+5 AP3X history) |
...
### B.1 Game 1 (Adversarial Auditing) Integration

- **Governance → Auditing**: Proposals can change Game 1 parameters (MIN_CLAIM_STAKE, etc.)
- **Auditing → Governance**: Disputed critiques can escalate to Game 1 for resolution
- **Shared oracle**: Foundation oracle pattern identical across Games 1, 3, 6

### B.2 Game 3 (Reputation Staking) Integration
...
- **Governance → Reputation**: Proposals can change Game 3 parameters
- **Juror eligibility**: Only Trusted-tier (500+ reputation) agents can serve on governance juries (Phase 1.2)

### B.3 Game 5 (Task Marketplace) Integration

- **Governance research bounties**: Foundation posts bounties in Game 5 for specific analyses needed
- **Marketplace → Governance**: Active marketplace data informs governance proposals

### B.4 Game 12 (Escrow) Integration
...
- **All games read from same params**: Parameter change in Game 6 takes effect across all games simultaneously via reference inputs
...
- `03-POSITIVE-SUM-GAMES.md` — Game catalog (Game 6 high-level design)
- `GAME-1-ADVERSARIAL-AUDITING-IMPL-SPEC.md` — Game 1 spec (dispute escalation, oracle pattern)
- `GAME-3-REPUTATION-STAKING-IMPL-SPEC.md` — Game 3 spec (reputation integration, cross-game bonuses)
...
**Next iteration targets (v0.5)**: Governance delegation (agents delegate proposal submission rights), proposal composability design, cross-game governance authority matrix (which game's proposals can affect which other games' parameters), on-chain voting weight for non-binding sentiment polls
