# Game 3: Reputation Staking — Implementation Specification

**Status**: DRAFT v0.4
**Author**: Filip Blagojević, with AI-assisted design
**Date**: 2026-03-20
**Dependencies**: Agent Registry contract (deployed), Game 1: Adversarial Auditing (for dispute escalation)
...
Reputation Staking is an economically-secured curation game where agents stake AP3X proportional to their claimed capabilities, and other agents endorse or challenge those claims. The result is a **self-curating directory** of trustworthy AI agents — the canonical place to discover reliable agents on Vector.

Your stake IS your reputation. Unstaking means losing credibility. This creates the **trust layer** in the Core Stack (Games 3 → 1 → 12 → 5).
...
Game 3 **extends** the existing Agent Registry — it does not replace it. The registry handles identity (DID, soulbound NFT, profile). Game 3 adds an economic layer on top:
...
│  - Game 1: Adversarial Auditing (dispute escalation)              │
│  - Protocol Params UTXO (shared with Game 1)                      │
└──────────────────────────────────────────────────────────────────┘
```
...
CHALLENGE (agent disputes another's claims):
  ...
  Resolution: automated verification, peer jury, or escalation to Game 1
...
DECAY (inactive agents lose reputation):
  ...
  This creates a Game 9 (Proof of Useful Work) synergy: agents can earn AP3X
...
pub type StakeDatum {
  ...
  last_updated: Int,
  /// Accumulated history bonus points (from winning challenges, cross-game bonuses)
...
/// in ANY game, creating contention on the stake UTXO).
///
/// Instead, activity tracking uses the INDEXER:
///   - The indexer monitors all game validator addresses
...
///     evidence (the agent's most recent tx in any game contract, provable via
///     reference inputs)
///
...
pub type RepChallengeState {
  /// Awaiting resolution
  Open
  /// Escalated to Game 1 Adversarial Auditing
...
pub type RepChallengeAction {
  ...
  /// Escalate to Game 1 for full adversarial resolution
  EscalateToAudit
...
```aiken
/// A history bonus UTXO — tracks permanent reputation earned from cross-game
...
pub type HistoryBonusDatum {
  ...
  agent_did: ByteArray,
  /// Source of the bonus (which game/event generated it)
...
pub type HistoryBonusSource {
  /// Won a capability challenge in Game 3
  ChallengeWon
  /// Won an audit challenge as claimer in Game 1
  AuditClaimWon
  /// Served as juror and voted with majority in Game 1
  JurorDuty
  /// Governance proposal adopted in Game 6
  ProposalAdopted
  /// Governance critique incorporated in Game 6
  CritiqueIncorporated
  /// Escrow task completed successfully in Game 12
  EscrowCompleted
  /// Useful work verified in Game 9
  UsefulWorkVerified
  /// Validated security report in Game 11
...
pub type HistoryBonusAction {
  /// Mint a new history bonus (called by game validators via cross-game bonus)
  MintHistoryBonus
...
Reputation decays if an agent is inactive. "Active" means the agent submitted any on-chain transaction (in any game) within the last `ACTIVITY_WINDOW` epochs.
...
This creates a Game 9 synergy: monitoring agents earn AP3X by processing reputation decay — useful work that maintains directory quality.
...
| Established | 100–499 AP3X | Meaningful stake | Eligible for Task Marketplace (Game 5) |
| Trusted | 500–1,999 AP3X | Significant economic backing | Eligible as juror (Game 1) |
...
MUST:
  ...
  2. Target agent is inactive — verified by one of:
     a. No transaction from target's credential appears in any game validator
        within ACTIVITY_WINDOW epochs (proven via reference inputs showing
        most recent game UTXOs for this agent are all older than threshold)
...
PHASE 1.0 SIMPLIFICATION:
  ...
  as Game 1 oracle). The Foundation periodically publishes an "inactivity
...
PHASE 1.1+:
  ...
  most recent UTXOs across all game validators are older than threshold.
```

### 6.7 Add History Bonus (`AddHistoryBonus`)

```
CALLABLE BY: Game validators (Game 1 challenge resolver, Game 9 work verifier, etc.)

MUST:
  1. Target StakeUTXO consumed (with stake token)
  2. A qualifying event occurred in a game validator (proven by consuming
     or referencing the game's resolution UTXO)
  3. bonus_amount calculated per cross-game bonus table (Section 8.3)
...
IMPLEMENTATION NOTE:
  ...
  - Cross-game bonuses can be BATCHED: a "bonus accumulator" UTXO
...
MUST:
  ...
  4. Creates a Game 1 Adversarial Auditing claim:
  ...
  6. Resolution handled entirely by Game 1 system
  7. Game 1 verdict maps back:
...
Phase 1.0 — Foundation Oracle:
  Same pattern as Game 1: Foundation multi-sig resolves.
...
Phase 1.1+ — Escalation to Game 1:
  For subjective capabilities (research, analysis):
  1. Either party calls EscalateToAudit
  2. Creates a claim in Game 1's Adversarial Auditing system
  3. Game 1 jury resolves the dispute
  4. Result feeds back to Game 3 for slashing/reward
...
Capabilities are free-text in the Agent Registry, but Game 3 needs structured capabilities for matching and challenges. Phase 1.0 uses a **curated vocabulary** stored in the ProtocolParams UTXO:
...
### 8.3 Automatic Reputation from Other Games

Agents earn implicit reputation through participation in other games:

| Game Activity | Reputation Effect |
|---------------|-------------------|
| Game 1: Won audit challenge (as claimer) | +10% of challenge stake as history bonus |
| Game 1: Served as juror (voted with majority) | +2 AP3X equivalent history bonus |
| Game 9: Verified useful work accepted | +5 AP3X equivalent history bonus |
| Game 6: Governance proposal adopted | +10 AP3X equivalent history bonus |
| Game 12: Escrow task completed successfully | +3 AP3X equivalent history bonus |

These cross-game bonuses are recorded via a `CrossGameBonus` UTXO minted by each game's validator when the qualifying event occurs. The reputation indexer includes them in the history bonus calculation.
...
All parameters governance-adjustable via ProtocolParams UTXO (shared with Game 1).
...
| `ESCALATION_WINDOW` | 5,400 | slots (~6h) | Window after response deadline to escalate to Game 1 |
...
│       ├── params.ak           # Protocol parameters (reference input, shared with Game 1)
...
pub type ReputationConfig {
  ...
  /// Script hash of Game 1 claim validator (for escalation)
...
/// across all games (Game 1, 3, 6, 11). It is NOT game-specific.
///
/// Game 3 sends AP3X to treasury in two scenarios:
...
///   Datum: TreasuryDatum { source_game: Int, last_deposit_slot: Int }
///
/// Treasury governance:
///   - Foundation controls withdrawals via multi-sig oracle
///   - Game 6 proposals can allocate treasury funds for specific purposes
///   - Treasury balance is public (any agent can query via indexer)
///   - Game 3 creates a new treasury output per deposit (no contention —
///     each deposit is a separate UTXO, similar to Game 6 batch pattern)
```

### 11.2b External Type Dependencies

The following types are defined in other contracts but referenced by Game 3 validators via reference inputs. Their definitions are reproduced here for completeness:

```aiken
/// ProtocolParams — shared across all games. Defined in the system-level
/// params contract. Game 3 reads via reference input at params_script_hash.
///
...
pub type ProtocolParams {
  ...
  jury_size: Int,
  jury_fee_rate: Int,
  /// Game 3 parameters (used by this contract)
  min_self_stake: Int,
  min_endorsement: Int,
  ...
  /// Game 6 parameters (not used by Game 3 directly)
  /// ... (omitted for brevity — see Game 6 spec §4.5)
...
/// Game 3 reads via reference input to verify capabilities and DID ownership.
///
...
```python
# vector_agent_sdk/games/reputation.py

class ReputationClient:
    """Client for Game 3: Reputation Staking"""
...
- If operator also owns the challenger: net zero minus protocol fee — same as Game 1 self-audit (-5% fees)
...
- The existence of Game 1 auditing infrastructure means challenge resolution is available.
...
Game 3 contributes to the AFI via:

| AFI Component | Measurement from Game 3 |
|---------------|------------------------|
...
### Phase 1.0 — Minimum Viable Reputation (+3 weeks from Game 1 launch)
...
- [ ] Foundation oracle for challenge resolution (same as Game 1)
...
- [ ] Escalation to Game 1 for dispute resolution
- [ ] Slashing logic for target + endorsers
- [ ] Decay mechanism and decay collector
- [ ] Cross-game bonus UTXOs (from Game 1, Game 9, Game 6)
...
- [ ] Integration with Game 5 (Task Marketplace) tier gating
- [ ] Integration with Game 12 (Escrow) auditor selection weighting
...
3. **Cross-game bonus calibration**: Are the bonus amounts in Section 8.3 correctly balanced? They create reputation from non-reputation activities — could this dilute staking incentives?
...
|------|----------|------------|------------|
| No one endorses (cold start) | High | Medium | Genesis Agent program, Foundation endorsements, cross-game bonuses |
...
| Cross-game bonuses issued | 5+ | 30+ | 100+ |
...
- **Escalation**: Reputation challenges can escalate to Game 1 for resolution

### B.2 Game 5 (Task Marketplace) Integration
...
### B.3 Game 9 (Proof of Useful Work) Integration
...
- `03-POSITIVE-SUM-GAMES.md` — Game catalog (Game 3 high-level design)
- `agent-infrastructure/contracts/agent-registry/` — Agent Registry contract (dependency)
- `GAME-1-ADVERSARIAL-AUDITING-IMPL-SPEC.md` — Game 1 spec (dispute escalation target)
